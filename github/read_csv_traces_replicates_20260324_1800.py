...
mcmc_or_dnax = 'mle' #should stay mle when looking at corrected2 results!
...
# PROCESS ONE MIXTURE
# =========================
def process_mixture(mixture_folder, mixture_type, dataset_number, replicate_number, sample, N_CONTRIBUTORS):
    ...
    mixture = f'combined_{mcmc_or_dnax}_{mixture_original}_{sample}' #f"combined_{dataset_number}{mixture_type}2_sample_1"
...
# MAIN
# =========================
def main1(mixture_folder,SAVE_FILES, version, CONFIG_YAML):
    #start = time.time()
    ...
    for mixture in mixture_list:
        ...
        RESULTS_DIR = Path(mixture_folder / rf"{mixture}\results_samples_{mcmc_or_dnax}_{version}")
        ...
        for sample in sample_list:
            ...
            result = process_mixture(mixture_folder,mixture_type, dataset_number, replicate_number, sample, N_CONTRIBUTORS)#, replicate_number)
            if result is None:
                continue
...
def save_filtered_dfs(percentile_df, results_base_dir, version, save_files):
    """
    ...
    for (mixture, contr), group in grouped:
        ...
        if save_files:
            results_dir = (
                ...
                / "mixtures"
                / mixture
                / f"results_samples_{mcmc_or_dnax}_{version}"
            )
            results_dir.mkdir(parents=True, exist_ok=True)
...
def create_filtered_dfs(mixture_folder, results_base_dir, version, save_files, CONFIG_YAML):
