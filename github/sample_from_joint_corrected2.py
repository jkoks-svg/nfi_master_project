...
def parse_vector_cell(x):
    """
    Parse a CSV cell containing a JSON vector, e.g. "[0.1, 0.2, 0.7]".
    """
...
def sample_joint_genotypes(df, N, phi_freq_dict, phi_genotype_dict, observed, DEBUG):
    """
    Sample full joint genotype profiles from a Bayesian vector deconvolution table.

    For each sampled profile:
      1) sample one MCMC draw index theta_idx;
      2) for every locus, sample one joint genotype row using
         Posterior_probability_draws[:, theta_idx];
      3) save theta_idx so that generation can use the same parameter draw.
    """

    COL_MARKER = "Locus"
    COL_PROB_DRAWS = "Posterior_probability_draws"

    df = df.copy()

    if COL_PROB_DRAWS not in df.columns:
        raise KeyError(
            f"Expected column {COL_PROB_DRAWS}. "
            "You are probably using an old scalar deconvolution CSV."
        )
    ...
    for locus, g in df.groupby(COL_MARKER):
        ...
        prob_mat = np.vstack([
            parse_vector_cell(x)
            for x in g[COL_PROB_DRAWS]
        ])

        if prob_mat.shape[1] == 0:
            raise ValueError(f"No MCMC draws found at locus {locus}")

        n_draws_set.add(prob_mat.shape[1])
        grouped.append((locus, g, prob_mat))
    ...
    for _ in range(N):
        theta_idx = int(np.random.randint(n_mcmc_draws))

        sample_dict = {
            "_metadata": {
                "theta_draw_index": theta_idx,
                "sampling_mode": "theta_first_joint_conditional",
            }
        }

        for locus, g, prob_mat in grouped:
            w = prob_mat[:, theta_idx].astype(float)
            w_sum = w.sum()

            if not np.isfinite(w_sum) or w_sum <= 0:
                raise ValueError(
                    f"Invalid posterior weights at locus {locus}, "
                    f"theta_idx={theta_idx}, sum={w_sum}"
                )
...
def write_samples(samples, output_root):
    ...
    for i, s in enumerate(samples, 1):
        ...
        metadata = s.get("_metadata", {})
        loci = [key for key in s.keys() if key != "_metadata"]

        if not loci:
            raise ValueError(f"Sample {i} contains no loci.")

        contributors = list(s[loci[0]].keys())

        # Save theta index used for genotype sampling.
        theta_path = sample_dir / "sampled_theta_index.json"
        ...
        for contributor in contributors:
            df = pd.DataFrame({
                "SampleName": contributor,
                "Marker": loci,
                "Allele1": [s[locus][contributor][0] for locus in loci],
                "Allele2": [s[locus][contributor][1] for locus in loci],
            })
