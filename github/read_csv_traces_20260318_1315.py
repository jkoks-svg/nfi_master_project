...
def prediction_interval_metrics(samples, observed, alpha=0.10):
    ...
    ----------
    samples : array-like
        Simulated/predicted log10 LR values.
    observed : float
        Observed true-donor log10 LR.
    alpha : float
        0.10 gives a central 90% prediction interval.
        0.05 gives a central 95% prediction interval.

    Returns
    -------
    dict
        lower, upper, width, covered, interval_score
    """
...
# PROCESS ONE MIXTURE
# =========================
def process_mixture(mixture_type, dataset_number, OUT_ROOT, N_GLOBAL, FILTERED_DF_BASE, N_CONTRIBUTORS,version):
    ...
    for info in infos: # best["infos"]:
        ...
        for replicate_number in replicate_numbers:
            filtered_df_path = FILTERED_DF_BASE / rf"{replicate_number}_{mixture}\results_samples_{mcmc_or_dnax}_{version_filtered}\filtered_df_{contr}_{version_filtered}.csv"
            ...
            # Compare simulated rework LR distribution (totals_on)
            # with the true donor log10 LR (true_log10_on)
            # pi90 = prediction_interval_metrics(
            #     samples=actual_rework_log10,
            #     observed=true_log10_on,
            #     alpha=0.10
            # )
            
            # Optional: also compute 95% interval metrics
            ...
            percentile_rows.append({
                ...
                # "pi90_lower": pi90["pi_lower"],
                # "pi90_upper": pi90["pi_upper"],
                # "pi90_width": pi90["pi_width"],
                # "covered_90": pi90["covered"],
                # "interval_score_90": pi90["interval_score"],
        ...
        if SHOW_MIXTURE_HISTOGRAMS:
            ...
            for replicate_number in replicate_numbers:
                path = FILTERED_DF_BASE / rf"{replicate_number}_{mixture}\results_samples_{mcmc_or_dnax}_{version_filtered}\filtered_df_{contr}_{version_filtered}.csv"
...
# def main():
    ...
    for mixture_type in MIXTURE_TYPES:
        for dataset_number in DATASETS:
            ...
            result = process_mixture(mixture_type, dataset_number, 
                                     OUT_ROOT, N_GLOBAL, FILTERED_DF_BASE, N_CONTRIBUTORS,version)#, replicate_number)
            if result is None:
                continue
    ...
    coverage_by_contributor = (
        ...
        .agg(
            ...
            # median_interval_score_90=("interval_score_90", "median"),
