...
# CONFIG
# =========================
SHOW_MIXTURE_HISTOGRAMS = False    # per-mixture ON vs corrected totals
...
#r"C:\Users\jortk\OneDrive\OneDrive Documenten\Thesis\Code_new\inputs\input_HT_2p_nodropin\output_HT_2p_nodropin_runF_traces5"
...
#OUT_ROOT = BASE /"inputs" /"input_HT_2p_nodropin"/"output_HT_2p_nodropin_runF_traces"

# # Console 5/A
#OUT_ROOT = BASE /"inputs" /"input_HT_2p_nodropin"/"output_DNASx_rework_frac_thresh_on_degr_on"

# # Console 8/A
# OUT_ROOT = BASE /"inputs" /"input_2p_Bayes"/"output_rework_MLE_degr_on"

# Console 11/A
#OUT_ROOT = BASE /"inputs" /"input_2p_Bayes"/"output_rework_MCMC_degr_on_500iters"
OUT_ROOT = Path(r"C:\Users\jortk\Documents\inputs\input_2p_Bayes\output_rework_MCMC_2p_corrected_v2")
...
#RAW_MIX_DIR = BASE / "inputs" / "input_HT_2p5p" / "mixtures"
...
def prediction_interval_metrics(samples, observed, alpha=0.05):
    """
    Coverage and interval score for an empirical predictive distribution.

    alpha=0.05 gives a 95% prediction interval.
    """
...
# PROCESS ONE MIXTURE
# =========================
def process_mixture(mixture_type, dataset_number):
    #mixture = f"{replicate_number}_{dataset_number}{mixture_type}2"
    mixture = f"{dataset_number}{mixture_type}2"
    ...
    for info in best["infos"]:
        ...
        pi95_on = prediction_interval_metrics(
            samples=totals_on,
            observed=true_log10_on,
            alpha=0.05
        )

        if False: #contr == 'Unknown2':
            #ADD SIMULATION ESTIMATE
            version_filtered = 'v1'
            replicate_numbers = [1,2,3]
            for replicate_number in replicate_numbers:
                filtered_df_path = Path(rf"C:\Users\jortk\OneDrive\OneDrive Documenten\Thesis\Code_new\inputs\input_HT_2p_nodropin\placeholder_output_runF_Qallele_fixed_100samples\mixtures\{replicate_number}_{mixture}\results_samples_{version_filtered}\filtered_df_{version_filtered}.csv")  
            ...
            percentile_rows.append({
                ...
                # NEW: 95% prediction interval coverage + interval score
                "pi95_lower": pi95_on["pi_lower"],
                "pi95_upper": pi95_on["pi_upper"],
                "pi95_width": pi95_on["pi_width"],
                "covered_95": pi95_on["covered"],
                "interval_score_95": pi95_on["interval_score"],
            })
        ...
        for mode in modes_to_run:
            ...
            if SHOW_MIXTURE_HISTOGRAMS:
                ...
                if False: #contr == 'Unknown2':
                    #replicate_numbers = [1]
                    for replicate_number in replicate_numbers:
                        filtered_df_path = Path(rf"C:\Users\jortk\OneDrive\OneDrive Documenten\Thesis\Code_new\inputs\placeholder_input_HT_2p_nodropin\output_runF_Qallele_fixed_100samples\mixtures\{replicate_number}_{mixture}\results_samples_{version_filtered}\filtered_df_{version_filtered}.csv")  
                        filtered_df = pd.read_csv(filtered_df_path)
                        mean_sim = filtered_df['true_log10_LR'].mean()
                        ax.hist(filtered_df['true_log10_LR'], bins=60, edgecolor="black", alpha=0.35, label=f"Simulations of mixture {replicate_number}_{mixture}")
                        ax.axvline(mean_sim, linestyle="--", linewidth=2, #color="green",
                                    label=f"Average simulations = {mean_sim:.2f}")
                ...
                ax.set_ylabel("Count")
                ax.set_title(f"Rework mixture {mixture} – {contr} (minor donor)")
                ax.legend()
                plt.show()
...
"SAVE_KS_TABLE": str(SAVE_KS_TABLE), # = True
    ...
    on_coverage_summary = (
        percentile_df[percentile_df["method"] == "ON"]
        .groupby(["contributor", "true_donor"])
        .agg(
            n=("covered_95", "size"),
    ...
    print("\n=======================================")
    print("   ON COVERAGE / INTERVAL SCORE SUMMARY")
    print("=======================================")
    print(on_coverage_summary.to_string(index=False))
    
    if SAVE_FILES:
        out = RESULTS_DIR / f"coverage_interval_score_ON_{version}.csv"
        on_coverage_summary.to_csv(out, index=False)
        print(f"[INFO] Saved ON coverage / interval-score summary -> {out}")
    ...
    if swap_records:
        for rec in swap_records:
            print(f"  {rec['mixture']} (dataset {rec['dataset']}): {rec['selected_mapping']}")
    ...
    if not outlier_loci_dfs:
        ...
        pct_keys = ["mixture", "dataset", "mixture_type", "contributor", "true_donor"]
...
if __name__ == "__main__":
    ...
    # # Remove NaN values
    # pct_values = percentile_df["pct_sim"].dropna()
    
    # # Plot histogram
    # plt.figure()
    # plt.hist(pct_values, bins=10)
    # plt.xlabel("pct_sim")
    # plt.ylabel("Frequency")
    # plt.title("Histogram of Unknown2 pct_sim (non-NaN values)")
    # plt.show()
    
    # # Remove rows where pct_sim is NaN
    # clean_df = percentile_df.dropna(subset=["pct_sim"])
    
    # # Extract variables
    # x = clean_df["percentile"].values
    # y = clean_df["pct_sim"].values
    ...
    # plt.ylabel("pct_sim")
    # plt.title("Scatterplot of Percentile vs pct_sim")
    
    # plt.show()
    
    if "pct_sim" in percentile_df.columns:
        pct_values = percentile_df["pct_sim"].dropna()
        ...
        if len(pct_values) > 0:
            ...
            if len(clean_df) >= 2:
                ...
                print("[INFO] Not enough pct_sim rows for Pearson correlation.")
    else:
        print("[INFO] pct_sim column not present; skipping pct_sim plots.")
        
        
#C:\Users\jortk\OneDrive\OneDrive Documenten\Thesis\Code_new\inputs\input_HT_2p_nodropin\output_HT_2p_nodropin_runF_traces5\mixtures
#C:\Users\jortk\OneDrive\OneDrive Documenten\Thesis\Code_new\inputs\input_HT_2p_nodropin\output_HT_2p_runF_traces5\mixtures\1A2