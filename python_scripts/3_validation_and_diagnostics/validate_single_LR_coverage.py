...
OUT_ROOT = Path(r"C:\Users\jortk\Documents\inputs\input_2p_Bayes\output_single_MCMC_2p_corrected_v2")
...
def log10_mean_exp_from_log10(log10_values):
    """
    Compute log10(mean(10**log10_values)) stably.

    This is the log10-scale version of:
        log BF = logsumexp(log LR_m) - log M
    """
...
def sample_N_genotypes_with_locus_log10(df, hypothesis, contributor, N=1000):
    ...
    Important:
      total_log10_LRs is not the sum of the locus summaries.
      It is log10(mean_m product_locus LR_locus,m).
    """
    sub = df[(df[COL_HYP] == hypothesis) & (df[COL_CONTR] == contributor)].copy()
    sub = sort_genotype_df(sub)
    ...
    if len(n_draws_set) != 1:
        raise ValueError(f"Not all loci have the same number of MCMC draws: {n_draws_set}")
    ...
    for i in range(N):
        # This theta draw is used only to sample a coherent multilocus genotype profile.
        ...
        for locus, prob_mat, log10_lr_mat in grouped:
            ...
            # Diagnostic only: locus-level Bayesian LR summary.
            # Do not sum these to get the total.
        ...
        # Correct final Bayesian averaging:
    ...
    for _, row in donor_df.iterrows():
        ...
        if best is None:
            ...
            if not fallback.empty:
                ...
                # Pick the fallback row with the highest locus-level Bayesian LR summary.
                # This is only for choosing between fallback candidates.
...
def process_mixture(mixture_type, dataset_number, replicate_number):
    ...
    # if missing_cols:
    #     raise KeyError(
    #         f"{results_path} is missing required Bayesian-vector columns: {missing_cols}"
    #     )
